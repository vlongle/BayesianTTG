...
// NOTE: everything is verbatim from Bandit except for proposalOutcome where we also add a regularizer!

// counting inversion functions here are identical to those in SignalSoftmaxQ.cpp
// O(N^2) but it is correct (do what I design)
double SignalBandit::countInversionOfProposal(Proposal &proposal)
{
...
VectorXd SignalBandit::calculateRegularizer(Agent &proposer)
{
...
pair<int, Proposal> SignalBandit::proposalOutcome()
{
    ...
    // dynamically add a soft regularizer to the proposaalValues
    // the bandit weights are not updated. Bandit weights are still updated through the usual procedure
    VectorXd regularizer = calculateRegularizer(proposer);

    double W = (proposer.proposalValues.sum() + regularizer.sum());
    VectorXd probs = (1 - gamma) * (proposer.proposalValues + regularizer);
    probs /= W;
...
pair<CoalitionStructure, vector<Coalition>> SignalBandit::formationProcess()
{
...
void SignalBandit::updateProposerBelief(Agent &proposer, Proposal &proposal, double coalitionReward)
{
    Coalition &coalition = proposal.first;
...
// response, 1 ==> 'yes', 0 ==> 'no'
void SignalBandit::updateResponderBelief(Agent &responder, Proposal &proposal, int response, double coalitionReward)
{
...
void SignalBandit::updateBelief(vector<Coalition> &nonSingletonCoals)
{
